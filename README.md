...
A token is an encrypted string that is created using a combination of the password provided and a secret key that only the backend has access to. In order to decrypt the token you would need the secret key. This gives a user security when logging in by preventing their password from falling into the wrong hands. We use these stored tokens to authenticate users and show them their private data upon successful login.

- [x] What steps can you take in your web apps to keep your data secure?

Ensure the user be authenticated before gaining access to private information. We can do this using private/protected routes. The user will gain access to the said information IF they are authenticated, if not they will be redirected to the login page. 

- [x] Describe how web servers work.

If you have an ISP connection in your house, or if you have a data plan on your device, you can access the internet. When you are browsing the internet, you are accessing information that is stored in servers. We gain access to this information via URL and domain name.

- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

C - stands for 'Create' which we gain access to via the POST method.
R - stands for 'Read' which we gain access to via the GET method.
U - stands for 'Update' which we gain access to via the PUT method.
D - stands for 'Delete' which we gain access to via the DELETE method.
